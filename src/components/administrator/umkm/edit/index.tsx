...
const EditForm = ({ id }: { id: string }) => {
    const { getUmkm, updateUmkm, updateUniqueTitle } = useUmkm();
    ...
    const handleSubmit = async (values: any) => {
        const success = await updateUmkm({
        ...
        if (success) {
            MySwal.fire('UMKM Berhasil Diubah!', '', 'success').then(() => {
    ...
    return (
        <>
        <h1 className="text-2xl font-semibold text-gray-900 mb-2">Edit UMKM</h1>
        <Formik
            enableReinitialize
        ...
        >
            <FormikForm className="space-y-6 p-6 bg-white rounded shadow-md">
                ...
                <button type="submit" className="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">Simpan Perubahan</button>
            </FormikForm>
        </Formik>
        </>
    );
